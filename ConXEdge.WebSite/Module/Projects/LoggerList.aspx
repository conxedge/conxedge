<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LoggerList.aspx.cs"
    Inherits="ConXEdge.WebSite.Module.Projects.LoggerList" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript"> 
        function addItem() {
            url = "LoggerAdd.aspx?rTime=" + (new Date().toString());
            window.location.href = url;
        }
        function editItem(pid) {
            url = "LoggerEdit.aspx?pid=" + pid + "&rTime=" + (new Date().toString());
            window.location.href = url;
        }
        ...
        function Delete(pid) {
            if (confirm("Do you confirm to delete logger?")) { 
                ...
                { sType: "delLogger", pid: pid, sDate: new Date().toString() },
                function (result) {
                    if (result == "") {
        ...
        //删除信息
        function RemoveBinding(pid) {
            if (confirm("Do you confirm to remove binding pour?")) {
                $.post("OperationHandler.aspx",
                { sType: "removeBinding", pid: pid, sDate: new Date().toString() },
                function (result) {
                    if (result == "") {
                        window.location.reload();
                    }
            ...
            <ItemTemplate>
                <tr>
                    ...
                    <td align="center" class="tc">
                        &nbsp;<%#Eval("PourName")%></td>
                    <td align="center" class="tc">
                        &nbsp;<%#GetOperation((string)Eval("LoggerID"), (string)Eval("PourName"))%>
                    </td>
                </tr>
            </ItemTemplate>
        </asp:Repeater>
