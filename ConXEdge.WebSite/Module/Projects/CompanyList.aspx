<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CompanyList.aspx.cs"
    Inherits="ConXEdge.WebSite.Module.Projects.CompanyList" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function addItem() {
            url = "CompanyAdd.aspx?rTime=" + (new Date().toString());
            window.location.href = url;
        }
        function editItem(pid) {
            url = "CompanyEdit.aspx?pid=" + pid + "&rTime=" + (new Date().toString());
            window.location.href = url;
        }

        //删除信息
        function Delete(pid) {
            if (confirm("Do you confirm to delete company?")) { 
                $.post("OperationHandler.aspx",
                { sType: "delCompany", pid: pid, sDate: new Date().toString() },
                function (result) {
                    if (result == "") {
...
<body>
    <div style="float: left; margin-top: 5px; margin-bottom: 5px; margin-left: 5px"> 
        <form id="form1" runat="server" method="get">
         Company：<input id="Company" name="Company" type="text" value="<%=Request.QueryString["Company"]%>" />
            ...
            <ItemTemplate>
                <tr>
                    ...
                    <td align="center" class="tc">
                        &nbsp;<%#Eval("CompanyName")%></td>
                    <td align="center" class="tc">
                        &nbsp;<%#ConvertType((string)Eval("CompanyType"))%></td>
                    <td align="center" class="tc">
                        &nbsp;<%#Eval("Logo")%></td>
                    <td align="center" class="tc">
                        &nbsp;<%#Eval("Style")%></td>
                    <td align="center" class="tc">
                        &nbsp;<%#Eval("Remark")%></td>
                    <td align="center" class="tc">
                        &nbsp;<a href='javascript:editItem("<%#Eval("CompanyID").ToString()%>");'>Edit </a>&nbsp;
                        <a href='javascript:Delete("<%#Eval("CompanyID").ToString()%>");'>Delete </a>
                    </td>
                </tr>
            </ItemTemplate>
        </asp:Repeater>
